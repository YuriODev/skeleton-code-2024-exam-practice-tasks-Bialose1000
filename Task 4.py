...
def Main():
  ...
  while Again == "y":
    ...
    if len(Filename) > 0:
      ...
      MyPuzzle = Puzzle(8, int(8 * 8 * 0.6)) #allows you to fill 60% of the grid before the puzzle finishes
  ...
  def __init__(self, *args):  # 
    if len(args) == 1:
      ...
      for Count in range(1, self.__GridSize * self.__GridSize + 1):
        if random.randrange(1, 101) < 90:   # 90% chance of putting a 2
          C = Cell()
        else:
          C = BlockedCell()
      ...
      self.__PatternCountQ = random.randrange(1, 3) ############
      XPattern = Pattern("X", "X*X*X*X*X")
      self.__AllowedPatterns.append(XPattern)
      self.__AllowedSymbols.append("X")
      self.__PatternCountX = random.randrange(1, 3) ############
      TPattern = Pattern("T", "TTT**T**T")
      self.__AllowedPatterns.append(TPattern)
      self.__AllowedSymbols.append("T")
      self.__PatternCountT = random.randrange(1, 3) ############
...
class Cell():
  def __init__(self):
    self._Symbol = ""
    self.__SymbolsNotAllowed = [] # This needs to have all the symbols that are not allowed
